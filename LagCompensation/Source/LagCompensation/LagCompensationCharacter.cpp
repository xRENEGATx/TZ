...
DEFINE_LOG_CATEGORY(LogLagCompensationCharacter);
...
ALagCompensationCharacter::ALagCompensationCharacter()
{
	...
	WeaponFireComponent->SetupAttachment(FP_Gun);
	WeaponFireComponent->SetRelativeLocationAndRotation(FVector(0.2f, 56.4f, 9.4f), FRotator(0.0f, 0.0f, 90.0f));
...
void ALagCompensationCharacter::OnFire()
{
	...
	{
		UE_LOG(LogLagCompensationCharacter, Warning, TEXT("WARNING: On fire called at server side. Aborted"));
		return;
	}
	...
	if (World != nullptr)
	{
		...
		{
			UE_LOG(LogLagCompensationCharacter, Error, TEXT("ERROR: WeaponFireComponent == nullptr"));
			return;
		}			
